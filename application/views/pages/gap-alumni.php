<link rel="stylesheet" href="<?= base_url('assets/css/gap-alumni.css') ?>" />

<main class="container">
    <div class="header">
        <h1>Graduate Agriprienur Program Winners 🏆</h1>
        <p>Celebrating Excellence in Agricultural Innovation</p>

        <div class="filters">
            <div class="school-filter">
                <label for="school">Filter by School:</label>
                <select id="school" class="school-select">
...
<script type="module">
    import {
        winnersData
    } from './assets/data/gap-alumni.js';
    ...
    function createExpandableContent(winner) {
        const expandableSections = winner.sections;
    ...
    // Updated function to render winners based on both school and year
    function renderWinners() {
        const container = document.getElementById('winners');
        const selectedSchool = document.getElementById('school').value;
        const selectedYear = document.getElementById('yearSelect').value;

        // Get winners for the selected school and year
        const schoolData = winnersData[selectedSchool];
        const winners = schoolData && schoolData[selectedYear] ? schoolData[selectedYear] : null;
        ...
        if (winners && winners.length > 0) {
            ...
            container.innerHTML = `
                    <div class="no-winners">
                        ...
                        <p>No winners found for ${selectedSchool} in ${selectedYear}</p>
                    </div>
                `;
        }
    }
